...
double M[101][101], pd[51][101][101];
...
int main()
{
    ...
    while(scanf("%d%d", &B, &N) && B) {
        ...
        pivot = c;
        sort(all(p), clockwise);
        for(i=0;i<N;i++) for(j=1;j<N;j++) {
            if(large_angle(p[i], c, p[(i+j)%N])) M[i][j] = INF;
            else {
                aux.clear();
                aux.pb(c);
                for(k=0;k<=j;k++) aux.pb(p[(i+k)%N]);
                M[i][j] = poly_area(convex_hull(aux));
            }
        }
        for(b=1;b<=B;b++) for(i=0;i<N;i++) for(j=0;j<N;j++) pd[b][i][j] = INF;
        for(i=0;i<N;i++) for(j=1;j<N;j++) pd[1][i][j] = M[i][j];
        for(b=2;b<=B;b++) for(i=0;i<N;i++) for(j=1;j<N;j++) for(k=1;k<j-1;k++)
            pd[b][i][j] = min(pd[b][i][j], pd[b-1][i][k] + M[(i+k+1)%N][j-k-1]);
        resp = INF;
        for(i=0;i<N;i++) resp = min(resp, pd[B][i][N-1]);
        printf("%.2lf\n", resp);
    }
    return 0;
}
