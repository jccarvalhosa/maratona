...
int proximo() {
    int prox;
    double d, dmin=INF;
    if(njog[0] == 0) return 0;
    if(njog[0] + njog[1] != 6) {
        if(njog[0] == 3) return 1;
        if(njog[1] == 3) return 0;
    }
    forr(i, 1, njog[0]+njog[1]) {
        d = abs(b[i], b[0]);
        if(d < dmin) dmin = d, prox = 1-ord[i];
    }
    return prox;
}
...
int main()
{
    int T, p, pontos;
    string nome[2];
    double x0, y0, teta, d, dmin;
    vector<pair<double, int> > v;
    scanf("%d", &T);
    while(T--) {
        cin >> nome[0] >> nome[1];
        scanf("%lf%lf%lf%lf", &x0, &y0, &teta, &d);
        b[0] = shot(pt(x0, y0), canonic_angle(teta), d);
        ord[0] = -1;
        njog[0] = njog[1] = 0;
        forr(i, 1, 6) {
            scanf("%lf%lf%lf%lf", &x0, &y0, &teta, &d);
            ...
            njog[ord[i]]++;
            b[i] = pt(x0, y0);
            b[7] = shot(b[i], canonic_angle(teta), d); 
            v.clear();
            v.pb(mp(d, 7));
            fori(j, i+1) if(between(b[i], b[j], b[7])) v.pb(mp(abs(b[j], b[i]), j));
            sort(all(v));
            fori(j, v.sz-1) b[v[j].second] = b[v[j+1].second];
        }
        p = proximo();
        pontos = 0;
        dmin = INF;
        forr(i, 1, 6) if(ord[i] == p) dmin = min(dmin, abs(b[i], b[0])); 
        forr(i, 1, 6) if(ord[i] == 1-p && cmpD(abs(b[i], b[0]), dmin) < 0) pontos++;
        cout << nome[1-p] << " " << pontos << endl;
    }
    return 0;
}
