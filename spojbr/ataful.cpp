...
//entender melhor (retorna 0 se os pontos sao colineares e 1 ou -1 dependendo de alguem estar a direita ou a esquerda)
int ccw(pt p, pt q, pt r) { return cmpD((p-r) % (q-r)); }

//verifica se o ponto p esta entre os pontos q e r
...
int main() {
    ...
    while(scanf("%lf %lf %lf",&x,&y,&r) && cmpD(r)){
        scanf("%lf %lf %lf %lf",&p,&q,&a,&t);
        printf("%.1lf\n", cannon_area(pt(p, q), pt(x, y), r, a, t));
    }
    return 0;
}
